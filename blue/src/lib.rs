...
// pub mod store {
//     include!(concat!(env!("OUT_DIR"), "/blue.store.rs"));
// }
...
// pub fn deserialize_store(path: &Path) -> Result<store::Store, prost::DecodeError> {
//     let store = match path.exists() {
//         true => {
//             let existing_store = fs::read(&path).unwrap();
//             store::Store::decode(&mut Cursor::new(existing_store.as_slice()))
//         }
//         false => Ok(store::Store::default()),
//     };
//     store
// }

// pub fn serialize_store(store: &store::Store) -> Vec<u8> {
//     let mut buf = Vec::new();
